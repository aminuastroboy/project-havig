...
def build_word_audio_map():
    audio_map = {}
    for item in LETTERS:
        audio_map[item["arabic"]] = make_audio_base64(item["word_ar"])
    return audio_map
...
st.markdown(
    """
    <div class="hero">
        <div class="pill">Hanif Learning Mode</div>
        <h1>Hanif's Arabic Alphabet Game 🧸</h1>
        <p>Tap any letter and it says the letter in Arabic. This version also teaches one simple Arabic word with an emoji for each letter.</p>
    </div>
    """,
    unsafe_allow_html=True,
)

mode_col, score_col = st.columns([1.3, 1], gap="large")
...
with score_col:
    st.markdown(
        f"""
        <div class="score-box">
            <div style="font-size:0.9rem; opacity:0.85;">Hanif's Progress</div>
            <div style="font-size:2rem; font-weight:800; margin-top:0.2rem;">⭐ {st.session_state.score}</div>
            <div style="font-size:1rem; opacity:0.9;">Streak: {st.session_state.streak}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
with left:
    if st.session_state.mode == "Learn":
        ...
        quiz_target = get_letter(st.session_state.quiz_target)

        st.markdown(
            f"""
            <div class="card">
                <div class="emoji">🎯</div>
                <div class="meta">Find this letter</div>
                <div class="big-letter">{quiz_target['arabic']}</div>
                <div class="meta">Tap the matching letter below</div>
            </div>
            """,
            unsafe_allow_html=True,
        )
            ...
            with quiz_cols[index % 4]:
                if st.button(item["arabic"], key=f"quiz_{item['arabic']}", use_container_width=True):
                    ...
                    if item["arabic"] == st.session_state.quiz_target:
                        ...
                        next_index = (
                            [i for i, x in enumerate(LETTERS) if x["arabic"] == st.session_state.quiz_target][0] + 1
                        ) % len(LETTERS)
...
st.write("1. In Learn mode, tap any letter.")
st.write("2. It says the Arabic name of the letter automatically.")
st.write("3. Tap Say Word to hear a simple Arabic word for that letter.")
st.write("4. Switch to Quiz mode when Hanif is ready.")
